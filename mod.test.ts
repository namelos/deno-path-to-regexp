...
import * as pathToRegexp from './mod.ts'
...
Deno.test('arguments should work without different call combinations', () => {
...
Deno.test('arguments should accept an array of keys as the second argument', () => {
  const keys: pathToRegexp.Key[] = []
  const re = pathToRegexp.pathToRegexp(TEST_PATH, keys, { end: false })

  assertEquals(keys, [TEST_PARAM])
  assertEquals(exec(re, '/user/123/show'), ['/user/123', '123'])
})

Deno.test('arguments should throw on non-capturing pattern', () => {
  assertThrows(() => {
    pathToRegexp.pathToRegexp('/:foo(?:\\d+(\\.\\d+)?)')
  }, TypeError, 'Pattern cannot start with "?" at 6')
})

Deno.test('arguments should throw on nested capturing group', () => {
  assertThrows(() => {
    pathToRegexp.pathToRegexp('/:foo(\\d+(\\.\\d+)?)')
  }, TypeError, 'Capturing groups are not allowed at 9')
})

Deno.test('arguments should throw on unbalanced pattern', () => {
  assertThrows(() => {
    pathToRegexp.pathToRegexp('/:foo(abc')
  }, TypeError, 'Unbalanced pattern at 5')
})

Deno.test('arguments should throw on unbalanced pattern', () => {
  assertThrows(() => {
    pathToRegexp.pathToRegexp('/:foo()')
  }, TypeError, 'Missing pattern at 5')
})

Deno.test('arguments should throw on missing name', () => {
  assertThrows(() => {
    pathToRegexp.pathToRegexp('/:(test)')
  }, TypeError, 'Missing parameter name at 1')
})

Deno.test('arguments should throw on nested groups', () => {
  assertThrows(() => {
    pathToRegexp.pathToRegexp('/{a{b:foo}}')
  }, TypeError, 'Unexpected OPEN at 3, expected CLOSE')
})
...
Deno.test('tokens should expose method to compile tokens to regexp', () => {
  const re = pathToRegexp.tokensToRegexp(tokens)
  assertEquals(exec(re, '/user/123'), ['/user/123', '123'])
})

Deno.test('token should expose method to compile tokens to a path function', () => {
  const fn = pathToRegexp.tokensToFunction(tokens)
  assertEquals(fn({ id: 123 }), '/user/123')
})

TESTS.forEach(test => {
  const [path, opts, tokens, matchCases, compileCases] = test

  const pathTestName = Deno.inspect(path) || 'empty'

  const keys: pathToRegexp.Key[] = []
  const re = pathToRegexp.pathToRegexp(path, keys, opts)

  if (typeof path === 'string') {
    Deno.test(`${pathTestName} should parse`, () => {
      assertEquals(pathToRegexp.parse(path, opts), tokens)
    })
    ...
    compileCases.forEach(function (io) {
      ...
      if (result !== null) {
        Deno.test(`should compile using ${Deno.inspect(params)}`, () => {
          assertEquals(toPath(params), result)
        })
      } else {
        Deno.test(`should not compile using ${Deno.inspect(params)}`, () => {
          assertThrows(() => {
            toPath(params)
          }, TypeError)
        })
    ...
    Deno.test('should parse keys', () => {
      assertEquals(keys, tokens.filter(token => typeof token !== 'string'))
    })
  }

  const matchTestName = `match${opts ? ` using ${Deno.inspect(opts)}` : ''}`

  matchCases.forEach(io => {
    const [pathname, matches, params, options] = io
    const message = `should ${
      matches ? '' : 'not '
    }match ${Deno.inspect(pathname)}`

    Deno.test(`${matchTestName} ${message}`, () => {
      assertEquals(exec(re, pathname), matches)
    })

    if (typeof path === 'string' && params !== undefined) {
      const match = pathToRegexp.match(path, options)

      Deno.test(`${matchTestName} ${message} params`, () => {
        assertEquals(match(pathname), params)
      })
    }
  })
})

Deno.test('compile errors should throw when a required param is undefined', () => {
...
Deno.test('compile errors should throw when it does not match the pattern', () => {
  const toPath = pathToRegexp.compile('/:foo(\\d+)')

  assertThrows(() => {
    toPath({ foo: 'abc' })
  }, TypeError, 'Expected "foo" to match "\\d+", but got "abc"')
})

Deno.test('compile errors should throw when expecting a repeated value', () => {
...
Deno.test('compile errors should throw when expecting a repeated value', () => {
  ...
  assertThrows(() => {
    toPath({ foo: [] })
  }, TypeError, 'Expected "foo" to not repeat, but got an array')
})

Deno.test('compile errors should throw when repeated value does not match', () => {
  const toPath = pathToRegexp.compile('/:foo(\\d+)+')

  assertThrows(() => {
    toPath({ foo: [1, 2, 3, 'a'] })
  }, TypeError, 'Expected all "foo" to match "\\d+", but got "a"')
})
